...
app.get('/', async (req, res) => {
    try {
        ...
        if (db) {
            ...
            const activeJobsResult = await db.query("SELECT COUNT(*) as count FROM scraping_jobs WHERE status IN ('pending', 'running', 'starting')");
            activeJobs = parseInt(activeJobsResult.rows[0]?.count || 0);
            
            // Get completed jobs from database
            const completedJobsResult = await db.query("SELECT COUNT(*) as count FROM scraping_jobs WHERE status = 'completed'");
...
app.post('/api/scraping/start', async (req, res) => {
    try {
        ...
        // Save job to database first and get the real ID
    ...
    try {
        ...
        const activeJobsResult = await db.query(`
            SELECT id, job_type, status, total_items, processed_items, 
                   success_count, progress, started_at, created_at
            FROM scraping_jobs 
        ...
        const recentHistoryResult = await db.query(`
            SELECT id, job_type, status, total_items, processed_items, 
                   success_count, completed_at, started_at
            FROM scraping_jobs 
        ...
        // Also include in-memory jobs for backward compatibility
        const memoryActiveJobs = Array.from(activeScrapingJobs.entries()).map(([id, job]) => ({
            id,
            status: job.status,
        ...
        res.json({
            ...
            memoryActiveJobs: memoryActiveJobs,
            totalActiveJobs: activeJobsResult.rows.length + memoryActiveJobs.length,
    ...
    try {
        ...
        for (let i = 0; i < job.zones.length; i++) {
            ...
            try {
                ...
                console.log(`Zone ${zone.display_name}: ${newRestaurants} nuevos, ${duplicateRestaurants} duplicados`);
            ...
            await db.query(`
                UPDATE scraping_jobs 
                SET processed_items = $1, success_count = $2, progress = $3, updated_at = NOW() 
                WHERE id = $4
            `, [job.processed, job.results, Math.round((job.processed / job.total) * 100), jobId]);
        ...
        await db.query(`
            UPDATE scraping_jobs 
            SET status = $1, completed_at = NOW(), processed_items = $2, 
                success_count = $3, progress = 100, updated_at = NOW() 
            WHERE id = $4
        `, ['completed', job.processed, job.results, jobId]);
        
        // Move to history
        scrapingHistory.push({ ...job });
        activeScrapingJobs.delete(jobId);
        
        console.log(`Job ${jobId} completed. Found ${job.results} new restaurants (${newRestaurants} nuevos, ${duplicateRestaurants} duplicados).`);
        ...
        await db.query(`
            UPDATE scraping_jobs 
            SET status = $1, completed_at = NOW(), error_messages = $2, updated_at = NOW() 
            WHERE id = $3
        `, ['failed', [error.message], jobId]).catch(dbErr => 
            console.error('Error updating failed job in DB:', dbErr)
        );
